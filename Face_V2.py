...
mydict = {}
for line in reader :
	mydict [line[0]] = {"number":int(line[1]),"gmail":line[2]}
...
def register():
	...
	Number = phone.get()
	Email = email.get()
	x = (frist_name in mydict.keys()) 
	if x is True :
		print(messagebox.showerror("Show Erorr", f"Already Exsit {frist_name} Try Again"))
		regFrame.destroy()
		#print(f"Already Exsit Welcome {name}")
	else :
		row_contents= []
		row_contents.append (frist_name)
		row_contents.append (Number)
		row_contents.append (Email)
		append_list_as_row('data.csv', row_contents)
		print(messagebox.showinfo("showinfo", f"Hello {frist_name} ,Your Phone Number is {Number} \n, and Your Email Address is {Email}"))
...
tk.Label(regFrame,text="Phone: ",font=("Arial",30),bg="white").grid(row=3,column=1)
phoneEntry=tk.Entry(regFrame,textvariable=phone,font=("Arial",30)).grid(row=3,column=2)

tk.Label(regFrame,text="Email : ",font=("Arial",30),bg="white").grid(row=4,column=1)
emailEntry=tk.Entry(regFrame,textvariable=email,font=("Arial",30)).grid(row=4,column=2)
